...
class Model(object):
    def __init__(self, device=[0]):
        ...
        elif cfg.mode=='semi_VL' or (cfg.mode=='semi_ST' and cfg.ST_with_infer):# semi-VL or semi-ST with inference model
    ...
    def pretrain(self,posterior=False):
        if cfg.mode=='train':
            ...
            if not os.path.exists(cfg.divided_path):
                ...
                self.post_data=self.reader.process_unlabeled_data(train_data[bound:]) # delete lablels
                ...
                for i, dial in enumerate(dial_batch_large):
                    ...
                    if len(dial_batch)==cfg.origin_batch_size or i==len(dial_batch_large)-1:
                        ...
                        try:
                            ...
                            outputs = self.model(inputs['contexts_tensor'])
                            if cfg.straight and not dial_batch_dict['supervised']:
    ...
    def semi_VL(self):
        ...
        if cfg.freeze_infer:
            for p in self.PosteriorModel.parameters():
        ...
        for epoch in tqdm(range(epoch_num)):
            ...
            for batch_idx, dial_batch_dict in enumerate(all_batches):
                ...
                if dial_batch_dict['supervised']==False:
                    st1=time.time()
                    if epoch==0 or (epoch>0 and not cfg.freeze_infer):
                        ...
                        if len(dial_batch)==cfg.origin_batch_size or i==len(dial_batch_large)-1:
                            try:
                                if cfg.mode=='semi_VL':
                                    ...
                                    if not cfg.freeze_infer:
                                        torch.nn.utils.clip_grad_norm_(self.PosteriorModel.parameters(), 5.0)
                                    epoch_step += 1
                                    dial_batch=[]
                                    epoch_step_uns+=1
                                elif cfg.mode=='semi_ST' and cfg.ST_with_infer:
                                    inputs_prior, labels_prior, bspn_labels_pri = \
                                        self.reader.convert_batch_session(dial_batch,only_resp_label=cfg.ST_resp_only,bspn_label=True)
                                    inputs_posterior, labels_posterior = \
                                        self.reader.convert_batch_session(dial_batch,posterior_train=True)
                                    ...
                                    loss=self.calculate_loss_and_accuracy(outputs1,labels_prior['contexts_tensor'])
                                    ...
                                    loss_uns+=loss.item()
                                    torch.nn.utils.clip_grad_norm_(self.PrioriModel.parameters(), 5.0)
                                    if cfg.straight and not cfg.freeze_infer:
                                        torch.nn.utils.clip_grad_norm_(self.PosteriorModel.parameters(), 5.0)
                                    epoch_step += 1
                                    dial_batch=[]
                                    epoch_step_uns+=1
            ...
            if cfg.evaluate_during_training:
                ...
                if eval_result['score']>max_score:
                    ...
                    self.save_model(posterior=True, path='best_post_model')
                    early_stop_count=cfg.early_stop_count
                else:
                    weight_decay_count-=1
                    if weight_decay_count==0 and not cfg.use_scheduler:
    ...
    def validate_fast(self,data='dev'):
        ...
        if not hasattr(self, 'PrioriModel'):
    ...
    def validate_pos(self,data='dev'):
        ...
        if os.path.exists(result_path) and 'test' in cfg.mode:
            ...
            eval_result['joint_acc'], eval_result['db_acc'], eval_result['slot_precision'], \
                eval_result['slot_recall'], eval_result['slot_F1'] = compute_jacc(results,return_db=True)
...
def main():
    ...
    cfg.divided_path=os.path.join(cfg.data_path,'divided_data{}_sd{}.json'.format(cfg.spv_proportion, cfg.seed))
